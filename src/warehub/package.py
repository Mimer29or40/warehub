...
from warehub.model import Directories, Project, Release, FileName, File
...
def add_package(package: Package) -> None:
    ...
    if has_signature:
        shutil.copy(package.signed_file, Config.path / Directories.FILES / package.signed_file.name)
    Database.commit()

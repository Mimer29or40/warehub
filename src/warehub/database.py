...
from typing import TypeVar, Any, Type, Union, Callable, Literal
...
class Encoder:
    ...
    def encode(self, obj: Any) -> dict[str, JsonValue]:
        ...
        return {'__type__': self.obj_type.__name__, 'value': self.encoder(obj), }
...
class TableEncoder(Encoder):
    ...
    def encode(self, obj: Any) -> dict[str, JsonValue]:
        """Encodes the python object into a JSON object"""
        return {'__type__': obj.__class__.__name__, 'value': self.encoder(obj), }
...
class TableField(Field):
    ...
    def __eq__(self, other: Any):
        try:
            ...
            return Comparison(lambda e: pattern.search(getattr(e, self.name)) is not None)
...
class Database:
    ...
    def commit(cls) -> bool:
        ...
        previous_commit = cls._data['last_commit'] if 'last_commit' in cls._data else None
        try:
            cls._data['last_commit'] = datetime.now()
            cls._file.write_text(json.dumps(cls._data, indent=2, default=cls._encode))
            return True
        except (IOError, Exception) as e:
            cls._data['last_commit'] = datetime.now() if previous_commit is None else previous_commit
            print(f'I/O error({e.errno}): {e.strerror}')
        return False
    
    @classmethod
    def get(cls, table: Type[TableType], where: ComparisonType = None) -> tuple[TableType]:
    ...
    @classmethod
    def pop(cls, table: Type[TableType], where: ComparisonType = None) -> tuple[TableType]:
        results: list[TableType] = []
    ...
    @classmethod
    def _get(cls, table: Type[TableType], where: ComparisonType = None) -> list[tuple[str, TableType]]:
        if cls._data is None:
            cls.rollback()
...
for register, coder in {
    (Database.register_encoder, Encoder(
        datetime,
        lambda datetime: datetime.isoformat()
    )),
    (Database.register_decoder, Decoder(
        datetime,
        lambda datetime_str: datetime.fromisoformat(datetime_str)
    )),
